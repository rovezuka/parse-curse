...
def get_data(url):
    ...
    # список данных каждого стартапа
    projects_data_list = []
    iteration_count = 23
    print(f"Всего итераций: #{iteration_count}")

    # количество страниц динамического сайта
    for item in range(1, 24):
        ...
        for project_url in projects_urls:
            ...
            project_name = project_url.split("/")[-2]  # достаем имя стартапа в ссылке
            ...
            # собираем всю информацию с каждого стартапа
            try:
            ...
            # через регулярное выражение удаляем теги, оставшиееся в описании
            ...
            # добавляем полученную информацию в список в виде словаря
    ...
    with open("data/projects_data.json", "a", encoding="utf-8") as file:
        json.dump(projects_data_list, file, indent=4, ensure_ascii=False)  # indent оступ, ensure кодировка
